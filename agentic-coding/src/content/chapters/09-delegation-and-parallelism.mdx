...
[Context as Currency](/agentic-coding/02-context-as-currency/) made the case that context decays non-linearly. The consequence — understated there, fleshed out here — is that the best response to "this task is too big for one session" is almost never "make the session bigger." It is "make the tasks smaller and run more of them."
