...
What you *don't* do on day one: coverage thresholds, exhaustive type hints, production-grade error handling. Inception phase is for shape-finding, not rigor. The anti-pattern from [Ch 11](/agentic-coding/11-antipatterns-recovery/) — premature rigor killing exploration — is specifically warning against front-loading this.
...
The big-bang refactoring anti-pattern from [Ch 11](/agentic-coding/11-antipatterns-recovery/) is the specific failure this protocol prevents. The counterpattern is four steps, applied repeatedly to small scopes:
...
When a refactor goes sideways, diagnose by layer — same pattern as the four-layer diagnosis from [Ch 11](/agentic-coding/11-antipatterns-recovery/), applied to refactoring specifically:
...
- Phase-appropriate rigor: every failure recipe in [Ch 11](/agentic-coding/11-antipatterns-recovery/) maps to a phase-mismatch.
