...
**Root cause:** multiple unrelated tasks in a single session. Each task's context remains, consuming tokens that contribute nothing to the current task. Covered in depth in [Ch 2 Context as Currency](/02-context-as-currency/).
...
**Prevention:** the two-failure rule (covered in [Ch 4 Session Loop](/04-session-loop/)). After two failed corrections, clear and re-prompt with precision.
...
**Prevention:** explicit phase transitions in the briefing doc, with graduation checklists and target dates. Covered in [Ch 5 Edit-Test-Commit](/05-edit-test-commit/).
...
**Prevention:** always provide verification criteria in the prompt, tests with code (not after), hook-enforced test requirements. Full treatment in [Ch 5 Edit-Test-Commit](/05-edit-test-commit/).
...
**Prevention:** incremental refactoring — extract one function, write characterization tests, refactor, verify, commit. Each step independently shippable. Characterization-test mechanics are covered in [Ch 5 Edit-Test-Commit](/05-edit-test-commit/); the full four-step refactoring protocol (extract → test → harden → promote) is covered in [Ch 10 Starting and Refactoring Projects](/10-starting-and-refactoring/).
