...
Each layer has Claude-Code instruments — `/memory` to see which instruction files actually load, and `/effort max` plus `/status` at the model layer. For those, and for how to smoke-test and adversarially test a rule (`settings.json` deny vs. advisory CLAUDE.md), see <BookLink book="handbook" to="chapters/ch14-antipatterns-recovery/#systematic-diagnosis-the-four-failure-layers">the Handbook's four-layer diagnosis</BookLink>.
