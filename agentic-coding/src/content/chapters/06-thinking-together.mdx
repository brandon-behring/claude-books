...
In [Ch 5](/05-edit-test-commit/), tests served verification: *does the code do what it claims?* Here, tests serve exploration: *what should the code do at all?*
...
<SkillBox title="Error messages that debug themselves">
  An error message that includes what went wrong, what was expected, what was received, and what the caller can do about it saves the agent the same investigation time it saves you. This is the fail-fast principle from [Ch 1](/01-agent-mental-model/) applied to agent collaboration: every error message is a tiny briefing doc for the next turn.
</SkillBox>
