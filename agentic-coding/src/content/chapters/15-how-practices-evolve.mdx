...
Convergence/divergence tracking turns a disorganized shelf of tool releases into a signal-extraction problem. The signal: which primitives have settled? Which are still contested? The book's own [changelog data](/agentic-coding/08-extending-agents/) is a concrete instance of this methodology — per-tool release timelines joined against a shared pattern registry to surface when multiple tools land the same primitive.
