...
- **Ask on request** (`-a on-request`) — the agent works autonomously on reads and analysis, asks before any write or destructive action.<Citation src="codex-cli-noninteractive" />
...
- Binary: `codex`; approval-mode flags (`-a on-request`, `--full-auto`, etc.) configure autonomy for non-interactive use
