...
sources:
  ...
  - lutke-context-engineering
  - karpathy-context-engineering
description: The argued opener for this book. The discipline that turns a model into an agent is the engineering of the two layers around it — the environment it acts in and the context it reasons over — and it is the most underappreciated, highest-leverage thing an architect designs.
---
...
Start with the gap this book is about. Code completion suggests the next token from the surrounding text; it has no autonomy. An **agent**, in Anthropic's framing, is a system where models "dynamically direct their own processes and tool usage."<Tag kind="official" /><Citation src="building-effective-agents" /> The working shorthand is tighter: agents are "LLMs autonomously using tools in a loop."<Citation src="effective-context-engineering" />
...
The coinage is young but independently converged-upon. Anthropic frames "context engineering" as curating what enters a finite window;<Tag kind="official" /><Citation src="effective-context-engineering" /> Shopify's Tobi Lütke independently prefers the term for "the art of providing all the context for the task to be plausibly solvable by the LLM,"<Tag kind="practitioner" /><Citation src="lutke-context-engineering" /> and Andrej Karpathy amplifies it as "the delicate art and science of filling the context window."<Tag kind="practitioner" /><Citation src="karpathy-context-engineering" /> A vendor and two practitioners name the same lever — not the prompt, not the model, but the context. <Tag kind="convergence" />
</Convergence>
