...
Across an 18-model panel and a peer-reviewed synthetic suite, the robust, corroborated finding is *that* performance falls with length and *that* the effective window is materially shorter than the claimed one — RULER reports "almost all models exhibit large performance drops as the context length increases."<Citation src="ruler" /> The specific percentages ("11 models drop below 50% of their strong short-length baselines" at 32K,<Citation src="nolima" /> "only half at 32K") are model- and benchmark-dependent.
