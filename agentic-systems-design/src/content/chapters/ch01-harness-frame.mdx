...
Start with the distinction that organizes everything else. An **agent**, in Anthropic's framing, is a system where models "dynamically direct their own processes and tool usage."<Tag kind="official" /><Citation src="building-effective-agents" /> The working shorthand is tighter still: agents are "LLMs autonomously using tools in a loop."<Citation src="effective-context-engineering" /> That is the **model** side — the source of the agent's autonomy.
