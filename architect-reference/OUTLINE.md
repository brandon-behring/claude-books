...
**What this is.** The cert-aligned volume of the claude-books series. It maps 1:1 to Anthropic's *Claude Certified Architect — Foundations* (CCA-F) five-domain taxonomy. **Exam-prep study guide.** As of Round 2 (2026-06-02) the genre was revised UP from terse exam-prep-reference to a **self-contained study guide** (Pearson/Sybex altitude): teach-from-scratch prose + a per-chapter "Do I Know This Already?" diagnostic, expanded worked examples, and retrieval practice with rationales — and **self-contained**, so the *Agentic Systems Design* links demote to optional **Further reading** rather than depth-deferral. **Rollout COMPLETE 2026-06-02 — all 30 chapters (D1.1–D5.6) converted** (Domain 1 pilot, then D2–D5 via `/loop`); every chapter now opens with a "Do I Know This Already?" diagnostic and carries at least one expanded `<WorkedExample>`.
...
**Status.** Round 1 — **PROSE-COMPLETE. All five Parts authored: Part I (D1.1–D1.7), Part II (D2.1–D2.5), Part III (D3.1–D3.6), Part IV (D4.1–D4.6), Part V (D5.1–D5.6) — 30 chapters, 36 pages, validate clean, all pushed to `origin/main`.** The REFERENCE sub-template is locked; **stable-principle** chapters are D3.5 (first) + D4.1; D5.3 introduced the book's first `<Tag kind="practitioner">` (Augment Code, T3). Manifest at **44 source keys**. Source backbone (per-chapter sources in `sources/manifest.yaml`) spans the 04/05/06/07/08/09/10 caches. **Round 2 (2026-06-02): genre revision COMPLETE.** An independent audit ([`docs/audits/2026-06-02_architect-reference-cert-book-independent-review.md`](../docs/audits/2026-06-02_architect-reference-cert-book-independent-review.md), conditional pass) found cert-depth (3.03/5) the binding weakness; the user then revised the genre **up** to a self-contained study guide, and **all 30 chapters were converted**: Domain 1 pilot (D1.1–D1.7, with the 3 previously-uncached D1.1 sources captured to `docs/research/`), then D2–D5 via `/loop "finish all the chapters"`. Each chapter folded its confirmed audit fix; **both hand-verified factual errors are fixed** (D5.3 "~7-week concurrency" → the union window Mar 4–Apr 20; D5.6 training-cutoff direction — the reliable cutoff is *earlier* than training, not later). The recurring official-tag-on-synthesis pattern was corrected (the schema-hook patterns are the book's D4.4 design synthesis, de-cited from `agent-sdk-structured-outputs` across D4.4/D5.5/D5.6). Every chapter builds clean (36 pages), passes `cert-audit.mjs` 11/11, and is committed but **not pushed**. Deferred-but-tracked (Round-2 tail): the fuller study-guide apparatus (pooled practice-exam, whole-book assessment test, glossary, flashcards, objective-map) needs scaffold features — see [`docs/scaffold-gaps.md`](../docs/scaffold-gaps.md); plus the 27 over-cap MarginNotes (advisory only) and cross-book XRef once the design book deploys (scaffold #96).
