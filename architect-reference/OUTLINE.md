...
**What this is.** The cert-aligned volume of the claude-books series. It maps 1:1 to Anthropic's *Claude Certified Architect — Foundations* (CCA-F) five-domain taxonomy. **Exam-prep study guide.** As of Round 2 (2026-06-02) the genre is being revised UP from terse exam-prep-reference to a **self-contained study guide** (Pearson/Sybex altitude): teach-from-scratch prose + a per-chapter "Do I Know This Already?" diagnostic, expanded worked examples, and retrieval practice with rationales — and **self-contained**, so the *Agentic Systems Design* links demote to optional **Further reading** rather than depth-deferral. **Piloted on Domain 1 (D1.1–D1.7);** D2–D5 remain on the Round-1 reference template pending rollout.
...
**Status.** Round 1 — **PROSE-COMPLETE. All five Parts authored: Part I (D1.1–D1.7), Part II (D2.1–D2.5), Part III (D3.1–D3.6), Part IV (D4.1–D4.6), Part V (D5.1–D5.6) — 30 chapters, 36 pages, validate clean, all pushed to `origin/main`.** The REFERENCE sub-template is locked; **stable-principle** chapters are D3.5 (first) + D4.1; D5.3 introduced the book's first `<Tag kind="practitioner">` (Augment Code, T3). Manifest at **44 source keys**. Source backbone (per-chapter sources in `sources/manifest.yaml`) spans the 04/05/06/07/08/09/10 caches. **Round 2 (2026-06-02): genre revision in progress.** An independent audit ([`docs/audits/2026-06-02_architect-reference-cert-book-independent-review.md`](../docs/audits/2026-06-02_architect-reference-cert-book-independent-review.md), conditional pass) found cert-depth (3.03/5) the binding weakness; the user then revised the genre **up** to a self-contained study guide. **Domain 1 pilot COMPLETE** — D1.1–D1.7 rewritten (DIKTA + teach-from-scratch + expanded `<WorkedExample>` + the confirmed audit fixes folded in; the 3 previously-uncached D1.1 sources captured to `docs/research/`). Awaiting pilot review before rolling out to D2–D5. Deferred-but-tracked: the fuller study-guide apparatus (per-chapter diagnostics, pooled practice-exam, whole-book assessment test, glossary, flashcards, objective-map) needs scaffold features — see [`docs/scaffold-gaps.md`](../docs/scaffold-gaps.md); plus cross-book XRef (scaffold #96).
