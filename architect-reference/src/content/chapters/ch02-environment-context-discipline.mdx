...
The coinage is young but independently converged-upon. Anthropic frames "context engineering" as curating what enters a finite window;<Tag kind="official" /><Citation src="effective-context-engineering" /> Shopify's Tobi Lütke independently prefers the term for "the art of providing all the context for the task to be plausibly solvable by the LLM,"<Tag kind="practitioner" /><Citation src="lutke-context-engineering" /> and Andrej Karpathy amplifies it as "the delicate art and science of filling the context window."<Citation src="karpathy-context-engineering" /> A vendor and two practitioners name the same lever — not the prompt, not the model, but the context. <Tag kind="convergence" />
</Convergence>
