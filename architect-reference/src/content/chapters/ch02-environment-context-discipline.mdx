...
<YouWillLearn prerequisites="The frame from the previous chapter — Agent = Model + Harness, and its three layers (environment, context-assembly, context).">
...
- A **map** of this part: the environment chapters, then the context chapters, and the problem→response→persistence arc that orders them
</YouWillLearn>

_The previous chapter drew the frame: an agent is a model plus a harness, and the harness owns three layers — environment, context-assembly, context. This chapter makes the case that two of those layers — the environment and the context — are where the discipline of building agents actually lives. It is the opener for the part that develops them; it argues the thesis and maps the chapters, it does not yet build anything._

## What autocomplete cannot do

Start with the gap the whole part is about. Code completion suggests the next token from the surrounding text; it has no autonomy. An **agent**, in Anthropic's framing, is a system where models "dynamically direct their own processes and tool usage."<Tag kind="official" /><Citation src="building-effective-agents" /> The working shorthand is tighter: an agent is an "LLM autonomously using tools in a loop."<Citation src="effective-context-engineering" />
...
<KeyIdea>
The model supplies autonomy; the **environment** and the **context** supply everything the autonomy can act on. Engineering those two layers — not choosing or prompting the model — is what turns autocomplete into an agent. That engineering is this part's subject.
</KeyIdea>
...
Treating environment and context as a single discipline is this part's framing, laid over the layers the sources describe. The sources name the layers; the unification is the argument.
</MarginNote>
...
The chapters develop the two layers in a deliberate order: **the substrate first, then the window** — and inside the window, **the problem before the response.**
...
<KeyIdea>
The arc is foundation-first, problem-first: build the environment, then confront why the window degrades, then engineer the window, then persist it. Each chapter develops one layer of the discipline this chapter names.
</KeyIdea>

## What is still settling

Two honest limits travel with this part, and they shape how to read it.
