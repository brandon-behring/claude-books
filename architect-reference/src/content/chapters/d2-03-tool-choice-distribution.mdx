...
<MarginNote variant="note" label="Note">
`strict: true` is available on all tools except `mcp_toolset`. It is the same prevention switch D2.2 uses against malformed inputs — here it composes with `any` to pin both *whether* a tool fires and *how* its inputs are shaped.
</MarginNote>
...
- **`any` + `strict: true` = a schema-valid guaranteed call** — `any` guarantees *a* tool fires, `strict: true` guarantees its inputs match the schema; compose them for classifiers/extractors. (`strict` is on all tools except `mcp_toolset`.)
