...
5. What are the three config `type` transports, and what is the default connection timeout?
...
A server's `type` selects its transport, and there are **three** config transports: `stdio` for local processes, `sse` (Server-Sent Events, now **deprecated** — use HTTP), and `http` (Streamable HTTP; JSON configs accept `streamable-http` as an alias for `http`).<Tag kind="official" /><Citation src="claude-code-mcp" /> Separately — and this is *not* a `.mcp.json` `type` — the SDK lets you run an MCP server **in-process inside your application** (an SDK deployment mode, e.g. a built-in tool server), rather than as an external process or endpoint.<Tag kind="official" /><Citation src="agent-sdk-mcp" />
...
- **Three config transports** — `stdio` / `sse` (deprecated) / `http` (`streamable-http` alias); the SDK in-process server is a separate deployment mode, not a `type`. The 2025-11-25 `initialize` handshake is a dated snapshot the 2026-07-28 RC removes.
