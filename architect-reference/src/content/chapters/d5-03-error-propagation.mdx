...
When multiple faults run at once, their aggregate is not the sum of their symptoms — and that is what makes them hard to catch. Anthropic's April-23 postmortem describes three production bugs running concurrently for roughly seven weeks, producing "broad, inconsistent degradation" that no single bug's symptom resembled; the most stubborn one "crossed context management, API header semantics, and the extended-thinking system."<Tag kind="official" /><Citation src="anthropic-april-23-postmortem" /> Detection was the central lesson: the bugs hit different traffic slices, and neither internal usage nor the existing eval suite reproduced them.<Tag kind="official" /><Citation src="anthropic-april-23-postmortem" />
