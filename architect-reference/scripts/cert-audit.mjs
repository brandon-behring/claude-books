...
const GLOSSARY_DIR = path.join(REPO_ROOT, 'glossary', 'terms');
const ASTRO_CONFIG_PATH = path.join(REPO_ROOT, 'architect-reference', 'astro.config.mjs');
...
// Check 15 — glossary integrity. The five CCA-F exam domains, mirrored verbatim from
// architect-reference/astro.config.mjs `examDomains` (a glossary term's optional `domain`
// must be one of these). Hand-synced, like VOLATILITY_ENUM.
const EXAM_DOMAINS = [
...
// The scaffold glossarySchema (.strict()) frontmatter keys — any other key fails the build.
const GLOSSARY_FRONTMATTER_KEYS = ['term', 'aliases', 'domain', 'see', 'tags'];
...
// ── Check 15: glossary integrity ─────────────────────────────────────────────
// The shared term layer's canonical source is glossary/terms/**.mdx (committed; the
// gitignored src/content/glossary/ copies are generated by scripts/sync-glossary.mjs).
// Read the canonical source and enforce the scaffold glossarySchema shape (required
// `term`, strict key set), `domain` in examDomains when present, and that every `see`
// id resolves to a real term. The <Term id> clause is dormant until the inline retrofit
// lands (it guards that follow-up): every <Term id="…"> in a chapter must resolve. If
// routes.glossary is on, the canonical source must be non-empty.
function checkGlossaryIntegrity(chapters) {
  const out = [];
  let glossaryRouteOn = false;
  try {
    glossaryRouteOn = /routes\s*:\s*\{[^}]*\bglossary\s*:\s*true\b/.test(read(ASTRO_CONFIG_PATH));
  } catch { /* config unreadable — treat the route as off */ }

  const files = walk(GLOSSARY_DIR, '.mdx');
  const ids = new Set(files.map((f) => path.basename(f, '.mdx')));

  if (glossaryRouteOn && files.length === 0)
    out.push(finding('FAIL', 'glossary/terms/', 'routes.glossary is true but no term files exist'));

  for (const f of files) {
    const name = `glossary/terms/${path.basename(f)}`;
    const { data } = parseFrontmatter(read(f));
    if (!data) { out.push(finding('FAIL', name, 'no parseable frontmatter')); continue; }
    if (typeof data.term !== 'string' || !data.term.trim())
      out.push(finding('FAIL', name, 'missing or empty `term`'));
    for (const key of Object.keys(data))
      if (!GLOSSARY_FRONTMATTER_KEYS.includes(key))
        out.push(finding('FAIL', name, `unknown frontmatter key "${key}" (glossarySchema is .strict())`));
    if (data.domain !== undefined && !EXAM_DOMAINS.includes(data.domain))
      out.push(finding('FAIL', name, `domain "${data.domain}" not in examDomains`));
    for (const sid of Array.isArray(data.see) ? data.see : [])
      if (!ids.has(sid)) out.push(finding('FAIL', name, `see id "${sid}" matches no glossary term`));
  }

  for (const ch of chapters)
    for (const m of ch.body.matchAll(/<Term\s+id=["']([^"']+)["']/g))
      if (!ids.has(m[1])) out.push(finding('FAIL', ch.name, `<Term id="${m[1]}"> matches no glossary term`));

  return { id: 15, title: `Glossary integrity (${ids.size} terms, see-links, <Term> ids)`, findings: out };
}
...
function main() {
  ...
  const results = [
    ...
    checkGlossaryIntegrity(chapters),
  ];
