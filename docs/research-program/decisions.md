...
- **Adopted (cheap discipline).** (a) cert-audit **Check 12** — a freshness tripwire on chapter `last_verified` vs `volatility`; **single source = chapter frontmatter** (no parallel ledger). (b) the **curriculum is a tracked source** in `topic-backlog.yml` (`model-lineup-flagship-cutover`, `cert-volatile-surface-living`): a book's volatile claims name the subjects that warrant live dossiers. (c) backing strict-live dossiers stay the single source of each fact (their `bib_ledger` `stale_after`). (d) an **adversarial-correctness gate** before stamping `last_verified` — *staleness ≠ correctness*: the linter + date catch stale/structure, not a fresh-but-wrong fact (the 2026-06-08 review caught one), so a re-run isn't done until an independent skeptic has checked the diff.
