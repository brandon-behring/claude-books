...
## D7 — The program is now LIVING for the curricula's volatile surface (2026-06-08)

- **Decision.** The program moves from "complete, fed the books once" to **continuously maintained** for the curricula's volatile facts. First live consumer: the cert book (`architect-reference`) — its `feature-surface` chapters' facts are tracked subjects, re-confirmed against live docs on a cadence.
- **Trigger (the "real trigger" the meta-pattern + D3 anticipated).** Opus 4.8 shipped 2026-05-28, six days after the cache; the 2026-06-08 factual-accuracy audit found the book *behind, not wrong* (model 4.7→4.8 cutover + count drift). That concrete pain justifies standing up the freshness loop now.
- **Adopted (cheap discipline).** (a) cert-audit **Check 12** — a freshness tripwire on chapter `last_verified` vs `volatility`; **single source = chapter frontmatter** (no parallel ledger). (b) the **curriculum is a tracked source** in `topic-backlog.yml` (`model-lineup-flagship-cutover`, `cert-volatile-surface-living`): a book's volatile claims name the subjects that warrant live dossiers. (c) backing strict-live dossiers stay the single source of each fact (their `bib_ledger` `stale_after`). (d) a **correctness gate = factored CoVe** (reuse the toolkit's `/dossier-audit` Phase 3 — decoupled skeptics blind to the edit; *not* a new bespoke review), made machine-visible by a `last_reviewed` chapter field + cert-audit **Check 13** (WARN when `last_verified` outruns it). *Staleness ≠ correctness*: Check 12 + the date catch stale/structure; Check 13 + CoVe catch fresh-but-wrong (the 2026-06-08 run shipped one).
- **Deferred (machinery).** Scheduled auto-audit + reviving the dormant weekly agent, a freshness dashboard, CI hard-gates, the per-claim→atom book-citation validator (D5), generalization to the handbook/design curricula. Full design + cadence: [`../design/2026-06-08_curriculum-live-dossier-loop.md`](../design/2026-06-08_curriculum-live-dossier-loop.md).
- **Status.** Resolved (2026-06-08). Extends the meta-pattern: this *is* the real trigger — adopt the tripwire + the tracked-source mapping; defer the automation.

## D8 — Source watch-map: the source-first layer (2026-06-17)

- **Decision.** Stand up a curated, **source-first** watch-map — [`source-registry.md`](./source-registry.md) — naming the standing sources worth monitoring for evolving best-practices / **code-samples** / direction, each with how-to-monitor · cadence · last-checked, plus a documented **method for finding new sources**. Source-first complements the topic-first taxonomy/trajectories; per-source freshness still lives in the dossiers' `bib_ledger.yml`.
- **Trigger.** The whole program is time-sensitive (weeks/months) and the `@claude`/`@anthropic-ai` YouTube channel proved a high-signal *direction* source (staff + partners reporting what's working) — a concrete need for a "what to watch + how to find new" map. **Discharges the shared source-registry D2 deferred** ("unless a wave reveals heavy cross-topic overlap" — it has: 21 sources shared by ≥2 dossiers) and **extends D7's living posture** from the cert curriculum to the whole source surface.
- **Adopted (cheap discipline).** The watch-map itself (markdown, curated ~50 feeds); a **new CODE & SAMPLES category** (the program's biggest gap — cookbook / quickstarts / agent-SDK / skills / plugins repos, all with commit/release `.atom` feeds); the channel uploads-feeds (UC IDs verified); a discovery method + cadence policy that reuse the `stale_after`/`freshness_tier` conventions + the [`cert-tracking.md`](../cert-tracking.md) weekly-diff pattern.
- **Deferred (machinery — per the meta-pattern).** The recurring **change-digest** ("what's new · what changed · new samples · impact on the books") and the scheduled `/schedule` cloud agent that wires the `[feed]`-marked sources + revives the dormant cert-tracking agent + the ROADMAP freshness heartbeat. The watch-map is the spec that automation would consume.
- **Status.** Foundation built 2026-06-17 (`source-registry.md`, links liveness-checked). Automation = next phase, on its own real trigger (a digest cadence to run).
