...
> **Revision — 2026-05-29 (Architect's Reference round).** The **default authoring tool is changed from Mermaid (Tier 1) to TikZ → SVG.** Rationale: output quality, author fluency, and the fact that the pipeline already ships (scaffold v4.2.0 `book-scaffold build-figures`) and the handbook's own figures are *already* authored in TikZ. The per-figure decision tree below now terminates at **TikZ as the default**, and **Mermaid is demoted to an optional fast-lane** for quick or contributor-authored structural sketches. The SVG-output, accessibility, dark-mode, build-time, and source/artifact-separation commitments are unchanged. Consequent scaffold task (tracked, cross-repo): harden the TikZ → SVG pipeline to inject `<title>`/`<desc>` accessibility and CSS-variable dark-mode fills, which TikZ does not emit natively. The Mermaid-default rationale below is retained as superseded history.
